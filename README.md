...
- `|merge %tasks our %garden` in fakezod
- `|mount %tasks` in fakezod
- `rm -rf ../zod/tasks/*` (empty out the tasks dir in your fakezod)
- from this dir (`tasks`), run `./copy-desk.sh zod` (you may have to `chmod +x copy-desk.sh` first)
- `|commit %tasks` in your fakezod
- `|revive %tasks` in your fakezod
- now to update the code, simply run `./copy-desk.sh` from the tasks dir and `|commit %tasks` in your fakezod


## UI

tasks is built primarily using [React], JavaScript, and [Tailwind CSS]. [Vite] ensures that all code and assets are loaded appropriately, bundles the application for distribution and provides a functional dev environment.

### Getting Started

To get started using tasks first you need to run `npm install` inside the `ui` directory.
...
4. From the `ui` directory you can run `rsync -avL --delete dist/ ~/zod/work/tasks` where `~/zod` is your fake urbit's pier.
...
7. You can now run `-make-glob %work /tasks` which will take the folder where you just added files and create a glob which can be thought of as a sort of bundle. It will be output to `~/zod/.urb/put`.
